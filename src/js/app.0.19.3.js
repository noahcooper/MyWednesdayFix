...
(function($) {
  ...
  if(window.location.href.indexOf('http:') !== -1) {
    ...
    $('body').on('click', 'a[href^="http:"], a[href^="https:"]', function(e) {
      ...
      alert('click, url = ' + $(this).attr('href'));
      
      myWednesdayFix.phonegap.inAppBrowser = window.open($(this).attr('href'), '_blank', 'location=yes');
      myWednesdayFix.phonegap.inAppBrowser.addEventListener('loadstart', function(loadStartEvent) {
        alert('loadstart, url = ' + loadStartEvent.url);
      });
    });
  }
